...
{ /* return 0: failure */
	...
	simxUChar inDat[SOCKET_MAX_PACKET_SIZE];
	simxShort inDatSize;
...
{ /* Returns the number of packets left to read if >=0, otherwise error */
	...
	{
		nb=extApi_recv_socket(clientID,packet+totalReceived,dataLength-totalReceived);
		if (nb<1)
			break;
		totalReceived+=nb;
...
SIMX_THREAD_RET_TYPE _communicationThread(simxVoid* p)
{
	...
	{ /* only the main thread can have this thread end! */
		...
		if (extApi_connectToServer_socket(clientID,_tempConnectionAddress[clientID],_tempConnectionPort[clientID])==1)
		{
			...
			while (_communicationThreadRunning[clientID]!=0)
			{
				...
				/* Send the message */
				if (_sendMessage_socket(clientID,tempBuffer,tempBufferDataSize)!=1)
				{
				...
				replyData=_receiveReplyMessage_socket(clientID,&replyDataSize);
				if (replyData==0)
					break;
...
EXTAPI_DLLEXPORT simxInt simxLoadScene(simxInt clientID,const simxChar* scenePathAndName,simxUChar options,simxInt operationMode)
{
	...
	{ /* give some pseudo-random name to the temp file: */
		...
		tmpFileName[22]='0'+(char)(extApi_rand()*9.1f);
		tmpFileName[23]='0'+(char)(extApi_rand()*9.1f);
		returnValue=simxTransferFile(clientID,scenePathAndName,tmpFileName,60000,simx_opmode_oneshot_wait);
		if (returnValue==0)
		{
			_exec_string(clientID,simx_cmd_load_scene,operationMode,0,(simxUChar*)tmpFileName,&returnValue);
			simxEraseFile(clientID,tmpFileName,simx_opmode_oneshot);
		}
		simxTransferFile(clientID,scenePathAndName,tmpFileName,60000,simx_opmode_remove);
	}
	else
...
JNIEXPORT jint JNICALL Java_coppelia_remoteApi_simxJointGetForce(JNIEnv *env, jobject obj, jint clientID, jint hdl, jobject frc, jint opMode)
{
	...
	simxInt retVal = simxJointGetForce(theClientID,jointHandle, &force, operationMode);
...
JNIEXPORT jint JNICALL Java_coppelia_remoteApi_simxGetStringSignal(JNIEnv *env, jobject obj, jint clientID, jstring sn, jobject sv, jint opMode)
{
	...
	if (retVal==0)
	{
		if (signalLength>0)
		{
			jclass cls = env->GetObjectClass(sv);
			jmethodID mid = env->GetMethodID(cls, "setValue", "(Ljava/lang/String;)V");
			simxUChar* sigValue = new simxUChar[signalLength];
			substr( (char*)signalValue, (char*)sigValue, 0, signalLength);
...
JNIEXPORT jint JNICALL Java_coppelia_remoteApi_simxSetStringSignal(JNIEnv *env, jobject obj, jint clientID, jstring sn, jstring sv, jint opMode)
{
	simxInt theClientID = clientID;
	const simxChar *signalName = (simxChar*)env->GetStringUTFChars(sn, 0);
	...
	simxInt retVal = simxSetStringSignal(theClientID,signalName, signalValue, signalLength, operationMode);
...
JNIEXPORT jint JNICALL Java_coppelia_remoteApi_simxQuery(JNIEnv *env, jobject obj, jint clientID, jstring sn, jstring sv,jstring rsn, jobject rsv, jint timeOutInMs)
{
	simxInt theClientID = clientID;
	const simxChar *signalName = (simxChar*)env->GetStringUTFChars(sn, 0);
	const simxUChar *signalValue = (simxUChar*)env->GetStringUTFChars(sv, 0);
	...
	simxInt retVal = simxQuery(theClientID,signalName, signalValue, signalLength,retSignalName,&retSignalValue,&retSignalLength,timeOutInMs_);

	if (retVal==0)
	{
		if (retSignalLength>0)
		{
			jclass cls = env->GetObjectClass(rsv);
			jmethodID mid = env->GetMethodID(cls, "setValue", "(Ljava/lang/String;)V");
			simxUChar* sigValue = new simxUChar[retSignalLength];
			substr( (char*)retSignalValue, (char*)sigValue, 0, retSignalLength);
			jstring s =  JNU_NewStringNative(env, (const char *)sigValue, retSignalLength);  //env->NewStringUTF((const char*)sigValue);
			env->CallVoidMethod( rsv, mid, s );			
			delete[] sigValue;
		}
	}
